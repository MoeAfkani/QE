#!/bin/bash
echo "This is an output file for $0 excuted in $(date)" &> $1.dew
##########################Constants##############################
lineOftotEnergy=1
lineOfP=1
lineOfForce=1
#################################################################
  for ecutwfc in `LANG=en_US seq $2 $4 $3`
    ...
    then
    # not conv
      echo $ecutwfc | tee -a $1.dew
    else 
    # conv
      let "lineOftotEnergy+=200"   #20 line upper than 1st time that "Energy" has observed! 
      echo -n "$ecutwfc"| tee -a $1.dew ;tail -$lineOftotEnergy $path/$ecutwfc.$1.scf.out | grep "!    total energy" |tail -c 20 | tee -a $1.dew
